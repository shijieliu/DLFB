 ...
 * @LastEditTime: 2020-07-11 15:32:03
 * @LastEditors: liushijie
 * @Description:
 * @FilePath: /LightLR/example/distributed_mnist.cpp
 */
#include "dl.h"
using std::vector;
using std::string;
...
int main() {
    ...
    std::unique_ptr<dl::dist::DistributedOptimizer> distributed_optimizer(new dl::dist::DistributedOptimizer(optimizer));
    ...
    for (int epoch = 0; epoch < n_epoch; ++epoch) {
        ...
        for (int n_step = 0; n_step < batch_imgs.size(); ++n_step) {
            distributed_optimizer->zeroGrad();
            dl::Time("forward", [&]() {
